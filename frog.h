...
/* Write msg into stderr and execute code in varargs */
#define REPORT(msg, ...)                                                      \
        do {                                                                  \
...
#define strcatf(strbuf, what, ...)                                            \
        sprintf ((strbuf) + strlen (strbuf), what, ##__VA_ARGS__)
...
#define frog_path_join(path, file)                                            \
        ({                                                                    \
                char *s = (char *)malloc (strlen (path) + strlen (file) + 2); \
                strcpy (s, path);                                             \
                strcat (s, "/");                                              \
                strcat (s, file);                                             \
                s;                                                            \
...
#define frog_cmd_async(command, ...)                                          \
        __frog_cmd_async (command, ##__VA_ARGS__, NULL)
int
...
#define frog_cmd_wait(command, ...)                                           \
        __frog_cmd_wait (command, ##__VA_ARGS__, NULL)
int
...
#define frog_rebuild_itself(argc, argv)                                       \
        do {                                                                  \
                if (frog_is_newer (__FILE__, argv[0])) {                      \
                        char new_name[32];                                    \
                        snprintf (new_name, sizeof new_name, OLD_NAME,        \
                                  argv[0]);                                   \
                        frog_cmd_wait ("mv", argv[0], new_name);              \
                        LOG ("Rebuilding " __FILE__ "\n");                    \
                        if (frog_cmd_wait ("gcc", __FILE__, "-o", argv[0],    \
                                           "-std=" FROG_STD, NULL)) {         \
                                PANIC ("Error "                               \
                                       "rebuilding\n");                       \
                        }                                                     \
                        frog_cmd_async ("rm", new_name);                      \
                        waitpid (frog_cmd_asyncl (argv[0], argv), NULL, 0);   \
                        exit (0);                                             \
                }                                                             \
        } while (0)

#define frog_cmd_foreach(command, ...)                                        \
        __frog_cmd_foreach (command, ##__VA_ARGS__, NULL)
void
__frog_cmd_foreach (frog_da_str files, const char *command, ...)
{
        int *pids = (int *)malloc (sizeof (int) * files.size);
        frog_da_str cmdargs;
        ZERO (&cmdargs);
        int i;
        char *sarg;
